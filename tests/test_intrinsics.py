...
@pytest.fixture
def camchain_path(tmpdir):
    path = Path(tmpdir) / ("camchain.yaml")
    path.write_text(camchain)
    return path


def test_camera_intrinsics_path(camchain_path):
    cam = CameraIntrinsics.from_kalibr_yaml(camchain_path, "cam0")
    _check_cam0(cam)


def test_camera_intrinsics_filename(camchain_path):
    cam = CameraIntrinsics.from_kalibr_yaml(str(camchain_path), "cam0")
    _check_cam0(cam)
